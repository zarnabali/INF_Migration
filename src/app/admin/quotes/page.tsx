...
const MOCK_QUOTES: Quote[] = [
  ...
  },
  {
    ...
    organization_name: 'Direct Sales',
    first_name: 'Mike',
    last_name: 'Johnson',
...
export default function QuotesPage() {
  ...
  const handleCreateOpen = () => {
    setCurrentStep(1);
    setCreateForm({
      firstName: 'Zarnab', // Pre-filled for demo as per screenshot
      lastName: 'Ali',
      email: 'zarnabalibbhatti@gmail.com',
      travelers: 3,
      tripCost: '5000',
      departureDate: '2026-01-23',
      departureCity: 'Phoenix, Arizona, US',
      product: 'MULTI-TRIP Basics (MTB) - Annual - $109.00',
      organization: 'My Travel Group',
      assignedUser: 'crispaul obana',
  ...
  const handleSaveQuote = () => {
    ...
    setTimeout(() => {
      const newQuote: Quote = {
        ...
        destination_city: 'Multiple', // Default for annual
        departure_date: createForm.departureDate,
        return_date: new Date(new Date(createForm.departureDate).setFullYear(new Date(createForm.departureDate).getFullYear() + 1)).toISOString().split('T')[0],
        travelers: createForm.travelers,
        trip_cost: parseFloat(createForm.tripCost),
        status: 'active',
  ...
  const filteredQuotes = useMemo(() => {
    return quotes.filter(quote => {
      ...
      // Org Filter
      if (orgFilter && quote.organization_name !== orgFilter) return false;
  ...
  }, [quotes, searchTerm, orgFilter, statusFilter, dateFrom, dateTo]);

  const hasActiveFilters = searchTerm || orgFilter || statusFilter.length > 0 || dateFrom || dateTo;
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-8">
      ...
      {/* Page Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[#005047] ">System Quote Management</h1>
        <p className="text-gray-500 mt-1">View and manage all quotes across the system</p>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                >
                  ...
                  <option value="Acme Travel Corp">Acme Travel Corp</option>
                  <option value="Globetrotter Inc">Globetrotter Inc</option>
              ...
              <div>
                ...
                <select
                  className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm"
                  multiple={false} // Keeping simple for now, simulated multiple would need UI component
          ...
          <table className="min-w-[900px] w-full">
            <thead>
              <tr className="bg-white border-b border-gray-100 text-left">
                ...
                <th className="py-4 px-6 text-sm font-semibold text-gray-600 w-[150px] hidden md:table-cell">Org</th>
                ...
                filteredQuotes.map((quote) => (
                  <tr key={quote.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-4 px-6 hidden md:table-cell">
                      <span className="text-sm text-gray-600 truncate max-w-[140px] block" title={quote.organization_name}>
                  ...
                  <td colSpan={8} className="py-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 mt-1">
                        {hasActiveFilters ? 'Try adjusting your filters' : 'No quotes have been created yet'}
                      </p>
                      {hasActiveFilters && (
                        <button
                          onClick={handleClearFilters}
                          className="mt-4 px-4 py-2 bg-white border border-blue-200 text-blue-600 font-medium rounded-lg hover:bg-blue-50 transition-colors"
                        >
                          Clear Filters
                        </button>
                      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-[16px] shadow-xl w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div>
                <h3 className="font-bold text-lg text-gray-800">Quote Details</h3>
                <p className="text-xs text-gray-500">{selectedQuote.id}</p>
              </div>
              <button onClick={() => setViewDialog(false)} className="text-gray-400 hover:text-gray-600">
                <IconX size={20} />
              </button>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <p><span className="text-gray-400 w-20 inline-block">Name:</span> {selectedQuote.first_name} {selectedQuote.last_name}</p>
                    <p><span className="text-gray-400 w-20 inline-block">Email:</span> {selectedQuote.email}</p>
                    <p><span className="text-gray-400 w-20 inline-block">Org:</span> {selectedQuote.organization_name}</p>
                  </div>
                </div>
                <div>
                  <h4 className="text-sm font-semibold text-gray-900 mb-3">Trip Details</h4>
                  <div className="space-y-2 text-sm text-gray-600">
                    <p><span className="text-gray-400 w-20 inline-block">Route:</span> {selectedQuote.departure_city} to {selectedQuote.destination_city}</p>
                    <p><span className="text-gray-400 w-20 inline-block">Dates:</span> {formatDate(selectedQuote.departure_date)} - {formatDate(selectedQuote.return_date)}</p>
                    <p><span className="text-gray-400 w-20 inline-block">Travelers:</span> {selectedQuote.travelers}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-xl flex justify-between items-center">
                <div>
                  <p className="text-xs text-blue-600 font-semibold uppercase tracking-wider mb-1">Total Premium</p>
                  <p className="text-2xl font-bold text-blue-900">{formatCurrency(selectedQuote.trip_cost)}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-medium text-blue-800">{selectedQuote.product_code}</p>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold capitalize ${selectedQuote.status === 'used' ? 'bg-green-100 text-green-700' : 'bg-white/50 text-blue-700'
                    }`}>
      ...
      {isCreateDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-[16px] shadow-xl w-full max-w-4xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
            ...
            {/* Stepper */}
            <div className="pt-8 pb-6 px-12 bg-[#F9FAFB] border-b border-gray-100 border-black/30 shrink-0">
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          type="number"
                          className="w-full pl-7 pr-4 py-2.5 rounded-md border border-blue-100 bg-[#eff6ff] focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm text-gray-900"
                    ...
                    <div className="col-span-2">
                      ...
                      <input
                        type="date"
                        className="w-full px-4 py-2.5 rounded-md border border-blue-100 bg-[#eff6ff] focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all text-sm text-gray-900"
